...
const UsageStats: React.FC<UsageStatsProps> = ({ useAdminPanel, username, className }) => {
    ...
    useEffect(() => {
        const fetchModels = async () => {
            try {
                const response = await usageApi.getModels(accessToken);
        ...
        try {
            ...
            try {
                const response = !useAdminPanel
                    ? await usageApi.getUserUsage(period, numPeriods, selectedModel, accessToken)
                    : !username || username === 'all'
                        ? await usageApi.getAllUsersUsage(period, numPeriods, selectedModel, accessToken)
                        : await usageApi.getSpecificUserUsage(username, period, numPeriods, selectedModel, accessToken)
